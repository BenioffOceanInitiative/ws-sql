...
SELECT
	...
	CASE 
		WHEN (coop_score_daily) >= 99 
			THEN 'A+'
		WHEN (coop_score_daily) < 99
			AND (coop_score_daily) >= 80 
			THEN 'A'
		WHEN (coop_score_daily) < 80
			AND (coop_score_daily) >= 60
			THEN 'B'
		WHEN (coop_score_daily) < 60
			AND (coop_score_daily) >= 40
			THEN 'C'
		WHEN (coop_score_daily) < 40
			AND (coop_score_daily) >= 20
			THEN 'D'
		ELSE 'F'
	...
	CASE 
		WHEN (ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) >= 99 
			THEN 'A+'
		WHEN (ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) < 99
			AND(ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) >= 80 
			THEN 'A'
		WHEN (ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) < 80
			AND(ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) >= 60
			THEN 'B'
		WHEN (ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) < 60
			AND(ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) >= 40 
			THEN 'C'
		WHEN (ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) < 40
			AND(ROUND((SAFE_DIVIDE(cum_sum_total_distance_nm_under_10 , cum_sum_total_distance_nm) * 100), 2)) >= 20 
			THEN 'D'
		ELSE 'F'
