...
CREATE TEMPORARY TABLE `temp_rgn_segs` (
  timestamp TIMESTAMP,
  date DATE,
  mmsi INT64,
  num INT64,
...
-- # Step 3: Query `whalesafe_v4.rgn_pts` and insert the constructed vessel segments into the temporary table
INSERT INTO `temp_rgn_segs`
  SELECT *,
  CASE 
  ...
  CASE WHEN	
    ST_INTERSECTS(
      ...
      (SELECT ST_UNION_AGG(geom) FROM ((
        SELECT * FROM `whalesafe_v4.shore`))))
	  THEN TRUE
	  ELSE FALSE
      ...
      FROM (
        ...
        FROM (
          ...
          FROM (
            ...
            FROM
              `whalesafe_v4.rgn_pts` AS ais
              LEFT JOIN
              `whalesafe_v4.ihs_data_all` AS ihs_data
...
--# Step 4: CREATE PARTITIONED AND CLUSTERED ` whalesafe_v4.rgn_segs` table IF NOT EXISTS
CREATE TABLE IF NOT EXISTS `whalesafe_v3.rgn_segs` (
  timestamp TIMESTAMP,
  date DATE,
  mmsi INT64,
  num INT64,
...
--# Step 5: Insert everything from temporary table into ` whalesafe_v3.rgn_segs`
INSERT INTO `whalesafe_v3.rgn_segs`
SELECT * FROM `temp_rgn_segs`;
