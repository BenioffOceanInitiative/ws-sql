...
CASE
WHEN
calc_knots BETWEEN 0.001 AND 50
AND
lag_calc_knots BETWEEN 0.001 AND 50
THEN TRUE
ELSE FALSE
END AS
good_point,
CASE WHEN
ABS(implied_speed_knots - speed_knots) > 8
AND
ABS(calc_knots - speed_knots) > 8
AND
...
AND DATE(ais.timestamp) >= ihs_data.start_date
AND DATE(ais.timestamp) <= ihs_data.end_date
WHERE
(timestamp) > new_seg_ts
AND
ihs_data.gt >= 300 -- # gross tonnage must be greater than or equal to 300
--AND good_seg IS TRUE
AND speed_knots > 0.001
AND implied_speed_knots > 0.001
AND overlapping_and_short IS FALSE
WINDOW w AS (PARTITION BY ais.mmsi ORDER BY timestamp)
)
WHERE dist_nm > 0 AND dist_nm <= 120
AND geom2 IS NOT NULL
)
WHERE
good_point IS TRUE
AND speed_diff_8 IS FALSE
)
