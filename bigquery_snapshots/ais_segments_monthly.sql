...
DECLARE snapshot_name STRING;
 ...
 SET snapshot_name = CONCAT("whalesafe_v3_backup.ais_segments_monthly_",
   FORMAT_DATETIME("%Y%m%d", @run_date));
 SET query = CONCAT("CREATE SNAPSHOT TABLE ", snapshot_name,
   " CLONE benioff-ocean-initiative.whalesafe_v3.ais_segments OPTIONS(expiration_timestamp=TIMESTAMP \"",
   expiration, "\");");
